...
function c_activate($c_username, $c_key) {
    ...
    $key_check = $c_con->query("SELECT * FROM c_keys WHERE c_key='".AntiSQLString($c_key)."'");
            ...
            if($c_row["c_used"] != 1) {
                ...
                if ($user_check->num_rows != 0) {
                    while ($c_row2 = $user_check->fetch_assoc()) {
                        ...
                        $c_con->query("UPDATE c_data SET c_expires='".AntiSQLString($time_to_update)."' WHERE c_username='".AntiSQLString($c_username)."'");
                        $c_con->query("UPDATE c_keys SET c_used='1' WHERE c_key='".AntiSQLString($c_key)."'");
